...
def test_ask_assistant():
    ...
    context = "context"
    mock_collection.query.return_value = {'documents': [[context]]}
    ...
    with patch('src.llm_client.ollama.chat') as mock_ollama:
        ...
        res = ask_assistant(mock_collection, mock_translator, original_query)
        mock_translator.translate.assert_called_once_with(original_query)
        mock_collection.query.assert_called_once_with(query_texts=translated_query, n_results = 1)
